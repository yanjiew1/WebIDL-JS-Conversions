...
type TypedArrayConstructors = typeof Int8Array | typeof Int16Array | typeof Int32Array | typeof Uint8Array |
  typeof Uint16Array | typeof Uint32Array | typeof Uint8ClampedArray | typeof Float32Array | typeof Float64Array;
...
const conversions: Conversions = {} as Conversions;

function makeException(ErrorType: typeof TypeError, message: string, options: ConversionOptions) {
  if (options.globals) {
    ErrorType = options.globals[ErrorType.name] as typeof TypeError;
  }
  return new ErrorType(`${options.context ? options.context : "Value"} ${message}.`);
}

function toNumber(value: unknown, options: ConversionOptions) {
  ...
  return options.globals.Number ? options.globals.Number(value as number) : Number(value);
}
...
function createIntegerConversion(bitLength: number, { unsigned }: { unsigned: boolean }) {
  ...
  return (value: unknown, options: IntegerConversionOptions = {}) => {
...
function createLongLongConversion(bitLength: number, { unsigned }: { unsigned: boolean }) {
...
conversions.double = (value, options: FloatingPointOptions = {}) => {
...
conversions["unrestricted double"] = (value, options: FloatingPointOptions = {}) => {
  const x = toNumber(value, options);

  return x;
};

conversions.float = (value, options: FloatingPointOptions = {}) => {
  const x = toNumber(value, options);
...
conversions["unrestricted float"] = (value, options: FloatingPointOptions = {}) => {
  const x = toNumber(value, options);
...
conversions.DOMString = (value, options: StringConversionOptions = {}) => {
...
conversions.ByteString = (value, options: StringConversionOptions = {}) => {
...
conversions.USVString = (value, options: StringConversionOptions = {}) => {
  return conversions.DOMString(value, options).toWellFormed();
};

conversions.object = (value, options: ConversionOptions = {}) => {
...
function isNonSharedArrayBuffer(value: unknown): value is ArrayBuffer {
  try {
...
function isNonSharedArrayBufferResizable(value: ArrayBuffer) {
  try {
...
function isSharedArrayBufferGrowable(value: SharedArrayBuffer) {
  try {
...
function isArrayBufferDetached(value: ArrayBuffer | SharedArrayBuffer) {
  try {
...
conversions.ArrayBuffer = (value, options: ArrayBufferOptions = {}) => {
...
conversions.SharedArrayBuffer = (value, options: ArrayBufferOptions = {}) => {
...
conversions.DataView = (value, options: ArrayBufferViewOptions = {}) => {
  try {
...
[
  ...
  conversions[name as TypedArrayNames] = (value, options: ArrayBufferViewOptions = {}) => {
    if (!ArrayBuffer.isView(value) || typedArrayNameGetter.call(value) !== name) {
      throw makeException(TypeError, `is not ${article} ${name} object`, options);
    }
    return conversions.ArrayBufferView(value, options) as ArrayBufferView;
  };
});

// Common definitions

conversions.ArrayBufferView = (value, options: ArrayBufferViewOptions = {}) => {
  ...
  if (!options.allowResizable) {
    if (isNonSharedArrayBufferResizable(value.buffer as ArrayBuffer)) {
      throw makeException(TypeError, "is a view on a resizable ArrayBuffer, which is not allowed", options);
    } else if (isSharedArrayBufferGrowable(value.buffer as SharedArrayBuffer)) {
...
conversions.BufferSource = (value, options: BufferSourceOptions = {}) => {
  if (ArrayBuffer.isView(value)) {
