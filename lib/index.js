...
function createIntegerConversion(bitLength, { unsigned }) {
  ...
  if (bitLength === 64) {
    lowerBound = unsigned ? 0 : Number.MIN_SAFE_INTEGER;
  ...
  return (value, options = {}) => {
    ...
    // % operator in EcmaScript always returns a result with the same sign as the dividend, which is different
    // from the modulo operation used in the spec.
    x = censorNegativeZero(x % twoToTheBitLength);
    if (unsigned) {
      if (x < 0) {
        // This will not work great for bitLength of 64, but oh well. See the README for more details.
        x += twoToTheBitLength;
      }
    } else {
      if (x >= twoToOneLessThanTheBitLength) {
        x -= twoToTheBitLength;
      } else if (x < -twoToOneLessThanTheBitLength) {
        x += twoToTheBitLength;
      }
    }
    return x;
...
conversions["long long"] = createIntegerConversion(64, { unsigned: false });
conversions["unsigned long long"] = createIntegerConversion(64, { unsigned: true });
